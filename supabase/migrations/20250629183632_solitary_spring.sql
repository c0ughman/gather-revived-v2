...
CREATE OR REPLACE FUNCTION increment(row_id UUID, increment_amount BIGINT)
RETURNS BIGINT
LANGUAGE SQL
AS $$
  SELECT COALESCE(
    (SELECT 
      CASE 
        WHEN column_name = 'call_time_used' THEN call_time_used
        WHEN column_name = 'agents_created' THEN agents_created
        WHEN column_name = 'integrations_active' THEN integrations_active
        WHEN column_name = 'storage_used' THEN storage_used
        WHEN column_name = 'chat_tokens_used' THEN chat_tokens_used
        ELSE 0
      END + increment_amount
    FROM user_usage, information_schema.columns
