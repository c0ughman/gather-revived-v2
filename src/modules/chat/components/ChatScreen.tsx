...
export default function ChatScreen({ 
  ...
  return (
    <div className="h-full bg-glass-bg flex flex-col font-inter">
      ...
      {/* Messages Area - Responsive width based on sidebar */}
      <div className={`flex-1 overflow-y-auto pt-20 pb-32 ${showSidebar ? 'pr-1/4' : ''}`}>
        <div className={`mx-auto px-4 ${showSidebar ? 'max-w-4xl' : 'max-w-6xl'}`}>
          {messages.length === 0 && (
      ...
      {showDocumentUpload && (
        <div className={`relative z-10 p-4 bg-glass-bg ${showSidebar ? 'pr-1/4' : ''}`}>
          <div className={`mx-auto ${showSidebar ? 'max-w-4xl' : 'max-w-6xl'}`}>
      ...
      {/* Input Area - Fixed at bottom with responsive width */}
