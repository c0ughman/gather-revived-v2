...
export default function PricingPage({ onSkipToPro }: PricingPageProps) {
  ...
  const { selectPlan, loading, error } = usePricing();

  const handleSelectPlan = async (planId: string) => {
    const result = await selectPlan(planId, billingCycle);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-black text-white">
      ...
      <div className="relative z-10 py-20">
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* Error Display */}
          {error && (
            <div className="max-w-md mx-auto mb-8 p-4 bg-red-900/50 border border-red-700 rounded-lg text-red-300 text-center">
              {error}
            </div>
          )}
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {pricingPlans.map((plan) => {
              const IconComponent = plan.id === 'standard' ? Zap : plan.id === 'premium' ? Crown : Rocket;
                ...
                >
                  ...
                  <div className="p-8">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className={`w-full py-3 px-6 rounded-full font-semibold transition-all duration-300 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed ${
                        plan.popular
          ...
          <div className="mt-20 max-w-3xl mx-auto">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
                ...
                <p className="text-slate-400">
                  We'll notify you when you're approaching your limits. You can upgrade your plan or wait for your usage to reset based on your billing cycle.
                </p>
              </div>
              <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
                <h3 className="text-lg font-semibold text-white mb-3">
                ...
                <p className="text-slate-400">
                  Yes! All paid plans come with a 14-day free trial. No credit card required to start with the free plan.
                </p>
              </div>
            </div>
          </div>
